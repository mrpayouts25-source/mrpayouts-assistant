...
async def receive_signal_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        parsed = parse_ctrader_link(link)
...
async def close_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    command = update.message.text.replace("/", "").strip().lower()
    context.user_data["close_result"] = command.upper()
...
async def receive_close_profit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if sent:
            await update.message.reply_text(
                f"✅ Trade {trade_number} closed as {result}.\nDuration: {duration}"
            )
        else:
...
async def receive_partial_percent(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        image_path = generate_update_image(
            trade,
            "Partial Secured",
            f"{percent}% secured. Runner left open."
        )
...
async def recap(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        recap_stats = calculate_recap(period)

        if recap_stats is None:
            await update.message.reply_text("Invalid recap period.")
            return
